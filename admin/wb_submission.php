...
if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in']==false) {
    ...
    if (isset($_GET['approve'])) {
        $sno = $_GET['approve'];
        $sql = "SELECT * FROM `website_submit` WHERE `sno` = $sno";
        ...
        if ($result) {
            ...
            echo '<div class="alert alert-danger container" role="alert">
                Some error occured in approving the submission! Please try again later.
            </div>';
        }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql = "UPDATE `website_submit` SET `email` = '$email', `name` = '$name', `type` = '$type', `heading` = '$heading', `submission` = '$submission' WHERE `sno` = '$sno'";
        $result = mysqli_query($conn, $sql);
...
?>
    ...
    <div class="main container">
        ...
        <div class="p-4"><a class="btn btn-primary" href="../submit.php" style="width: 100%;">New Submission</a></div>
        <div class="container table-responsive table-responsive-xl">
            <table class="table table-bordered" id="myTable" style="table-layout: fixed; width: 100%">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT * FROM `website_submit` ORDER BY sno DESC";
                    $res = mysqli_query($conn, $sql);
                    $srno = 0;
                    while ($row = mysqli_fetch_assoc($res)) {
                        $srno = $srno + 1;
                    ?>
                        <!--Card Start-->
                        <tr>
                            <!-- substr($result_fetch['content'],0,35) -->
                            ...
                            <td>
                                <button class='approve btn btn-sm btn-success m-1' id="
        ...
        Array.from(approves).forEach((element) => {
            element.addEventListener("click", (e) => {
                ...
                if (confirm("Are you sure you want to approve this submission!")) {
                    console.log("yes");
                    window.location = `./wb_submission.php?approve=${sno}`;
                } else {
                    console.log("no");
                }
            });
